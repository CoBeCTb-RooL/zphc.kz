...
$sliderPresents = $MODEL['sliderPresents'];
	...
	<div class="index-main-slider">
		...
		<div class="unslider">
			<ul >
			  ...
			  if($sliderPresents )
			  {?>
			  <li class="index-slider-presents zoom-on-hover slide1" style="background-image: url(<?='/'.UPLOAD_IMAGES_REL_DIR.''.$sliderPresents->attrs['pic']?>); background-position: center center; ">
			  	
			  		<div class="info"><?=$sliderPresents->attrs['descr']?></div>
					
					<div><a href="<?=$sliderPresents->url()?>" class="btn">Перейти к подаркам</a></div>
			  </li>
			  <?php 	
			  }?>
			  <!-- /слайдер-ПОДАРКИ -->
