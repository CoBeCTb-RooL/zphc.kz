...
if(count($items))
{?>
<form action="/<?=ADMIN_URL_SIGN?>/course/itemsListSubmit" target="frame7">
	<div style="padding: 10px 0 0 0px; font-size: 16px; ">Курсов: <b><?=$MODEL['totalCount']?></b></div>
	...
	<table class="t">
		<tr>
			...
			<th>Товары</th>
			<th>ред.</th>
			<th><a href="#" onclick="OPTS.orderBy='idx'; OPTS.desc='<?=$orderBy=='idx' ? ($desc ? 0 : 1) : 0?>'; itemsList(); return false; "><nobr>Порядок <?=$orderBy=='idx' ? ($desc ? $downArrowSign : $upArrowSign) : '' ?></nobr></a> </th>
			<th>удалить</th>
		</tr>
		<?php 
		//vd(Course::$fields)?>
	...
	//vd($item->url())?>
		<tr id="item-<?=$item->id?>" class="item-status-<?=$item->status->code?>">
			...
			{?>
				<div style="margin: 0 0 3px 0; white-space: nowrap;  "> <?=++$i?>. <a href="<?=$p->product->url()?>" target="_blank"><?=$p->product->name?></a></div>
			<?php 	
			}?>
			</td>
...
<p><button type="button" onclick="itemEdit(''); ">+ Добавить курс</button>
