...
<form id="item-edit-form" enctype="multipart/form-data" method="post" action="/<?=ADMIN_URL_SIGN?>/presents/itemEditSubmit" target="frame81" onsubmit="itemEditSubmitStart(); " >
...
?>
	...
	<div class="field-wrapper">
		...
		<span class="value" >
			...
			<div style="max-height: 300px; overflow-y: auto; width: 370px; display: inline-block;   ">
				<h3>Триггеры</h3>
			<?php 
			...
			{?>
				...
				$quans = array();
				foreach($MODEL['item']->triggerProducts as $p)
				{
				...
				{?>
							<div class="product" style="margin: 0 0 7px 40px; vertical-align: middle; ">
								<label>
									<input type="checkbox" name="products[<?=ProductRelationType::PRESENT_TRIGGER?>][<?=$product->id?>]"  style="vertical-align: middle; " <?=(in_array($product->id, $relatedProducts) ? ' checked="checked" ' : '')?> />
									...
									<span class="product-title" style="display: inline-block; vertical-align: middle; "><?=$product->name?></span>
									 &times;<input type="text" name="quans[<?=ProductRelationType::PRESENT_TRIGGER?>][<?=$product->id?>]" value="<?=$quans[$product->id]?>"  style="width: 15px; "/>
								</label>
							</div>
				<?php 	
				}?>
			...
			{?>
				...
				$relatedProducts = array();
				$quans = array();
				foreach($MODEL['item']->presentProducts as $p)
				{
					$relatedProducts[] = $p->productId;
					$quans[$p->productId] = $p->param1;
				}
				...
				{?>
							<div class="product" style="margin: 0 0 7px 40px; vertical-align: middle; ">
								<label>
									<input type="checkbox" name="products[<?=ProductRelationType::PRESENT?>][<?=$product->id?>]"  style="vertical-align: middle; " <?=(in_array($product->id, $relatedProducts) ? ' checked="checked" ' : '')?> />
									<a href="/<?=UPLOAD_IMAGES_REL_DIR?><?=$product->photo?>" class="highslide" onclick="return hs.expand(this)" style="vertical-align: middle; "><img src="<?=$product->photo ? Media::img($product->photo) : Funx::noPhotoSrc()?>&width=50" alt="<?=$product->name?>" /></a> 
									<span class="product-title" style="display: inline-block; vertical-align: middle; "><?=$product->name?></span>
									 &times;<input type="text" name="quans[<?=ProductRelationType::PRESENT?>][<?=$product->id?>]" value="<?=$quans[$product->id]?>"  style="width: 15px; "/>
								</label>
							</div>
				<?php 	
				}?>
