<?php
class DiscountsController extends MainController{
	
	
	
	function routifyDiscount()
	{
		global $CORE;
	...
	function index()
	{
		...
		//vd(Discount::$fields);
		Core::renderView('discounts/indexView.php', $model);
	}
	...
	function itemsList()
	{
		...
		Core::renderView('discounts/itemsListIndex.php', $MODEL);
	}
	...
	function itemsListAjax()
	{
		...
		{
			$elPP = Discount::ADMIN_ELEMENTS_PER_PAGE;
			...
			//vd($params);
			$MODEL['items'] = Discount::getList($params);
			foreach($MODEL['items'] as $c)
				$c->initRelatedProducts();
			//vd($MODEL['items']);
			$params['limit'] = '';
			$MODEL['totalCount'] = Discount::getCount($params);
		}
		else
			$MODEL['error'] = Error::NO_ACCESS_ERROR;
	
		Core::renderView('discounts/itemsListAjax.php', $MODEL);
	}
	...
	function itemEdit()
	{
		...
		if($ADMIN->hasRole(Role::SYSTEM_ADMINISTRATOR))
		{
			$MODEL['item'] = Discount::get($_REQUEST['id']);
			if($MODEL['item'])
				$MODEL['item']->relatedProducts = ProductRelation::getList(ProductRelationType::code(ProductRelationType::DISCOUNT), $MODEL['item']->id);
		...
		Core::renderView('discounts/itemEdit.php', $MODEL);
	}
	...
	function itemEditSubmit()
	{
		...
		if($ADMIN->hasRole(Role::SYSTEM_ADMINISTRATOR))
		{
			if(!$item = Discount::get($_REQUEST['id']))
				$item = new Discount();
			...
			# 	сохраняем продукты
			ProductRelation::deleteRelations(ProductRelationType::code(ProductRelationType::DISCOUNT), $item->id);
			foreach(array_keys($_REQUEST['products']) as $v)
			{
				//vd($v);
				$rel = new ProductRelation();
				$rel->productId = $v;
				$rel->relationType = ProductRelationType::code(ProductRelationType::DISCOUNT);
